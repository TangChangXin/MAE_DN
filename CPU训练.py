    ...
    for 当前训练周期 in range(1, 命令行参数.max_epoch):
        ...
        for 训练批次, 每名患者全部图像块 in 训练循环:
            ...
            for 每个图像块 in 每名患者全部图像块:
                每个图像块 = torch.div(每个图像块, 255) # 类似transforms.ToTensor()
                每个图像块 = transforms.Normalize(0.5, 0.5, inplace=True)(每个图像块)
                每个图像块 = 每个图像块.to(硬件设备)
                融合图像 = 网络模型(每个图像块) # 融合图像形状[1*16*16]

                拼接图像.append(融合图像) # 这里要把输出的融合图像拼接起来，现在不直接拼成一张完整的图，直接以列表形式保存。
                # TODO 尝试在这里拼接图像转换为[1, 361, 16, 16]
                # print(融合图像)
