...
def 训练模型(融合图像模型, 重建图像模型, 优化器, 硬件设备, 命令行参数):
    ...
    for 当前训练周期 in range(1, 命令行参数.max_epoch):
        ...
        for 训练批次, 每名患者全部图像块 in 训练循环:
            ...
            # TODO
            """
            实际上每个图像块的中间计算结果数据并没有销毁，依旧存在于显存或者是内存之中，所以显存占用其实并不会下降。
            现在尝试将原始数据按照[64, 224, 224]的大小划分 相乘大小5,914,624
            唐晓颖论文中描述是112*224*112，相乘大小2,809,856
            """
            拼接图像.to(硬件设备)
            # 预测值, 掩码 = 重建图像模型(拼接图像)
...
if __name__ == '__main__':
    ...
    嵌入向量维度 = 512
    立体网络模型, 平面重建模型 = model.无重建融合网络(注意力头数量=8).to(硬件设备), model.直接自监督重建OCTA图像().to(硬件设备)
    # 立体网络模型, 平面重建模型.to(硬件设备)
